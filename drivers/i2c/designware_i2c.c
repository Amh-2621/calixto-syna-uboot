 ...
 */
static int dw_i2c_calc_timing(struct dw_i2c *priv, int mode,
			  ...
			  ulong bus_clk, struct dw_i2c_speed_config *config)
{
	...
	if (scl_sda_cfg) {
		...
		ret = dw_i2c_calc_timing(priv, i2c_spd, bus_clk, 0,
					 config);
		if (ret) {
			printf("%s, timing calc failed ret = %d\n", __func__, ret);
			return ret;
		}
	}
...
static int _dw_i2c_set_bus_speed(struct dw_i2c *priv, struct i2c_regs *i2c_base,
				 unsigned int speed, unsigned int bus_mhz)
{
	struct dw_i2c_speed_config config;
	unsigned int cntl;
	unsigned int ena;
...
static int designware_i2c_probe(struct udevice *bus)
{
	...
	debug("dw_i2c: scl_rise_time_ns = %d scl_fall_time_ns = %d sda sda_hold_time_ns = %d\n",
	      priv->scl_rise_time_ns, priv->scl_fall_time_ns, priv->sda_hold_time_ns);
